...
namespace Sorter.Core.MapReduce
{
    public class Reducer
    {
        ...
        private string _keysPattern = @"(sorted_)?map_([^_]+)_?(.+)?.txt";
        ...
        public void Run()
        {
            while (Directory.GetFiles(_dataDirectory).Length > 1)
            {
                ...
                Parallel.ForEach(source.OrderBy(s => s).Batch(5), fileNames =>
                {
                    ...
                    var from = fromMatches.First().Groups[2];
                    var toMatches = Regex.Matches(fileNames.Last(), _keysPattern).Last();
                    var to = string.IsNullOrWhiteSpace(toMatches.Groups[2].Value) ? toMatches.Groups[3] : toMatches.Groups[2];
